...
if($_POST){
	...
	if(count($greska)==0){
		unset($_POST["sifra"]);
		$izraz=$veza->prepare("insert into fakultet (naziv, puni_naziv, kontakt, grad, drzava) 
							values (:naziv, :puni_naziv, :kontakt, :grad, :drzava);");
		$izraz->execute($_POST);
		    ...
		    <div class="form-group col-md-4">
		     	<?php if(!isset($greska["puni_naziv"])): ?>
						  <label>Puni naziv Fakulteta
						    <input class="form-control"  type="text" id="puni_naziv" name="puni_naziv" placeholder="Fakultet elektrotehnike i računarstva"
